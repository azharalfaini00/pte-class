...
export const OwnerDashboard: React.FC = () => {
  const { 
    pricing, updatePriceItem, updateSettings, updateTutor, addTutor, removeTutor, 
    ...
    uploadImage, savePricing, formatRupiah 
  } = usePricing();
  ...
  const handlePriceChange = (category: keyof PricingState, id: string, field: 'basePrice' | 'discountPercentage', value: string) => {
    const numValue = parseInt(value.replace(/\D/g, ''), 10) || 0;
    
    const item = pricing[category].find((i: any) => i.id === id);
    if (!item) return;

    if (field === 'basePrice') {
      updatePriceItem(category, id, numValue, item.discountPercentage);
    } else {
      let safePercent = numValue > 100 ? 100 : numValue;
      updatePriceItem(category, id, item.basePrice, safePercent);
    }
  };
  ...
  const renderPricingTable = (category: keyof PricingState, title: string, subtitle: string) => (
              ...
              return (
                <tr key={item.id} className="hover:bg-slate-50/50 transition-colors group">
                  ...
                  <td className="px-6 py-5 text-right">
                    <span className="font-bold text-indigo-600 bg-indigo-50 px-3 py-1.5 rounded-lg">
